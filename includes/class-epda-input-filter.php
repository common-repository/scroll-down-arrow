...
class EPDA_Input_Filter {
	...
	public function validate_and_sanitize_specs( array $input, array $specification ) {
		...
		if ( empty( $input ) ) {
			return new WP_Error( 'invalid_input', esc_html__( 'Error Occurred', 'scroll-down-arrow' ) . ' (5532)' );
		}
		...
		foreach ( $input as $key => $input_value ) {
			...
			switch ( $type ) {
				...
				case self::WP_EDITOR:
					$input_value = wp_kses( $input_value, EPDA_Utilities::get_extended_html_tags() );
					break;
			...
			if ( is_wp_error( $result ) ) {
				...
				EPDA_Logging::add_log( 'Please change the value of ' . $field_spec['label'] . ' field. Current value: "' . $input_value . '" - ' . $result->get_error_message() . ', code: ' . $result->get_error_message(), $result );
				...
				if ( ( empty($field_spec['internal']) && count($errors) > 0 ) || ( defined('SCRIPT_DEBUG') && SCRIPT_DEBUG ) ||
					...
					$errors[] = '<div style="padding: 20px 0 20px 0;">'. sprintf( esc_html__( 'Please change value of the %s field.', 'scroll-down-arrow' ), $lang ) . ' ' . $result->get_error_message() . '</div>';
	...
	private function filter_input_field( $value, $field_spec ) {
		...
		switch ( $field_spec['type'] ) {
			...
			default:
				return new WP_Error('epda-invalid-input-type', esc_html__( 'Error Occurred', 'scroll-down-arrow' ) . ' - ' . $field_spec['type']);
		}
	}
	...
	private function filter_text( $text, $field_spec ) {
		...
		if ( strlen($text) > $field_spec['max'] ) {
			...
			$msg = sprintf( _n( 'The value is too long. Remove %d character.', 'The value is too long. Remove %d characters.', $nof_chars_to_remove, 'scroll-down-arrow' ), $nof_chars_to_remove );
		...
		if ( ( empty($text) && ! empty($field_spec['mandatory']) ) || ( strlen($text) > 0 && strlen($text) < $field_spec['min'] ) ) {
			...
			$msg = sprintf( _n( 'The value is too short. Add at least %d character.', 'The value is too short. Add at least %d characters.', $nof_chars_to_remove, 'scroll-down-arrow' ), $nof_chars_to_remove );
	...
	private function filter_select( $value, $field_spec ) {
		...
		if ( ! in_array( $value, array_keys($field_spec['options']) )  && ! empty($field_spec['mandatory']) ) {
			$value_text = ( empty($value) ? esc_html__( 'empty', 'scroll-down-arrow' ) : '"' . $value . '"' );
			/* translators: 1: value entered by user, 2: HTML tag, 3: valid values user should enter. */
			$msg = '<br>' . sprintf( esc_html__( 'The value cannot be %1$s. %2$sValid values are: %3$s', 'scroll-down-arrow' ), $value_text, '<br>', implode(", ", $field_spec['options']) );
	...
	private function filter_checkbox( $value ) {
		...
		/* translators: %s: value entered by user. */
		return new WP_Error('filter_checkbox_invalid', sprintf( esc_html__( 'The value "%s" is not valid', 'scroll-down-arrow' ), $value ) );
	}
	...
	private function filter_number( $number, $field_spec=array() ) {
		...
		$number_int = EPDA_Utilities::sanitize_int( $number, null );
		if ( $number != $number_int ) {
			/* translators: %s: value entered by user. */
			return new WP_Error('filter_not_number',sprintf( esc_html__( 'The value "%s" is not a number', 'scroll-down-arrow' ), $number ) );
		}

		if ( $number > $field_spec['max'] ) {
			/* translators: 1: value entered by user, 2: maximum length user should enter. */
			$msg = sprintf( esc_html__( 'The value %1$s is larger than maximum of %2$s', 'scroll-down-arrow' ), $number, $field_spec['max'] );
			...
			$msg = sprintf( esc_html__( 'The value %1$s is smaller than minimum of %2$s', 'scroll-down-arrow' ), $number, $field_spec['min'] );
			return new WP_Error( 'filter_not_number', $msg );
		}
	...
	private function filter_true_false( $boolean ) {
		...
		return new WP_Error( 'filter_not_number', esc_html__( 'Error Occurred', 'scroll-down-arrow' ) . ' (112)' );
	}
	...
	public function filter_color_hex( $value, $field_spec=array() ) {
		...
		/* translators: %s: value entered by user. */
		return new WP_Error('filter_not_color_hex', sprintf( esc_html__( 'The value "%s" is not a valid HEX color. Expected format: #xxxxxx', 'scroll-down-arrow' ), $value ));
	}
	...
	private function filter_id( $id ) {
		$id = EPDA_Utilities::sanitize_get_id( $id );
		if ( is_wp_error($id) ) {
			return new WP_Error('filter_not_id', esc_html__( 'Error Occurred', 'scroll-down-arrow' ) . ' - ' . $id . ' - ' . $id->get_error_code() );
		}
		return $id;
	}
	...
	private function filter_enumeration( $value, $field_spec ) {
		...
		return new WP_Error('filter_not_enumeration', esc_html__( 'Error Occurred', 'scroll-down-arrow' ) . ' - ' . $value );
	}
	...
	public function retrieve_and_sanitize_form_fields( $submitted_fields, $all_fields_specs, $orig_settings ) {
		...
		foreach ($all_fields_specs as $key => $spec ) {
			...
			if ( $spec['type'] == self::WP_EDITOR ) {
				$name_values += array( $key => wp_kses( $input_value, EPDA_Utilities::get_extended_html_tags() ) );
