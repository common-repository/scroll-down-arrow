...
class EPDA_Logging {
	...
	const LOGGING_OPTION_NAME =  'epda_error_log';
	...
	public static function add_log( $error_message, $param1='', $param2=null ) {
		...
		$variable = EPDA_Utilities::get_variable_string( $variable );
		...
		// prepare error message
		$error_message = EPDA_Utilities::substr( $error_message, 0, 3000);
		...
		// add new error log entry but remove the oldest one if more than max
		$error_log[] = array( 'plugin' => EPDA_PLUGIN_NAME, 'date' => gmdate("Y-m-d H:i:s"), 'message' => $error_message, 'trace' => $stack_trace );
		...
		// save the error log
		EPDA_Utilities::save_wp_option( self::LOGGING_OPTION_NAME, $error_log );
	}
	...
	public static function get_logs() {
		$logs = EPDA_Utilities::get_wp_option( self::LOGGING_OPTION_NAME, array(), true );
	...
	private static function can_log_message() {
		...
		$is_debug_on = EPDA_Utilities::get_wp_option( 'epda_debug', false );
	...
	public static function generateStackTrace()	{
		...
		foreach( debug_backtrace() as $trace ) {
			...
			$file = EPDA_Utilities::substr($file, 1, 500);
		...
		$stackMsg = EPDA_Utilities::substr( $stackMsg, 0, 2000);
